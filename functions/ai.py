...
class Ai:
    ...
    def __init__(self, logging_filename):
        # Need to open a logging file
        self.logging_file = open(logging_filename+".log" + "w+")
        self.logging_file.write("Created Game log\t" + get_now_formatted())
    ...
    def minimax(self, chessboard, depth, alpha, beta, maximizing, memo, finals):
        """
        ...
        if maximizing:
            ...
            for start, move_set in black_moves.items():
                for end in move_set:
                    ...
                    # last but not least, if this piece gets into a threatened position, I'll reduce the boards value
                    if (piece.y, piece.x) in attacked:
                        # / 2 so that it considers to make sacrifices
